...
#ifndef _SIMCOM_SSL_H_
#define _SIMCOM_SSL_H_
...
typedef struct{
    ...
    UINT8* client_cert;
    UINT8* client_key;
    UINT8  ca_cert_path[256];       //Compatible file system, prevent file name overflow
    UINT8  client_cert_path[256];
    UINT8  client_key_path[256];
    UINT8  enable_SNI;
    UINT8  auth_mode;
    UINT8  ignore_local_time;
    UINT16 negotiate_time;
#ifdef FEATURE_SIMCOM_TLS_1_3
    UINT8 use_tickets;  /*!< use session tickets?  0 disable, 1 enable, default 0 */
#endif
}SCsslContent;
...
#else
  SC_SSL_CFG_VERSION_TLS12=0,                        /* tls 1.2 */
  SC_SSL_CFG_VERSION_TLS13,                          //TLS1.3
#endif
