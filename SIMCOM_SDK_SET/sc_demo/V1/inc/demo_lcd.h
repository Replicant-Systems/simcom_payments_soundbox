/**
  ******************************************************************************
  * @file    demo_lcd.h
  * @author  SIMCom OpenSDK Team
  * @brief   Header file of lcd demo,including all ASCII code with 8x16\ST7735S & ST7789V cmd list\some chinese characters.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2022 SIMCom Wireless.
  * All rights reserved.
  *
  ******************************************************************************
  */
#ifndef _DEMO_LCD_H_
#define _DEMO_LCD_H_

//ASCII 8x16
static const unsigned char ASCII_8X16[]={
 0x00,0x00,0x00,0x18,0x24,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x24,0x18,0x00,0x00,//"0",0
 0x00,0x00,0x00,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x00,0x00,0x18,0x18,0x00,0x00,//"!",1
 0x00,0x12,0x36,0x24,0x48,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,//""",2
 0x00,0x00,0x00,0x24,0x24,0x24,0xFE,0x48,0x48,0x48,0xFE,0x48,0x48,0x48,0x00,0x00,//"#",3
 0x00,0x00,0x10,0x38,0x54,0x54,0x50,0x30,0x18,0x14,0x14,0x54,0x54,0x38,0x10,0x10,//"$",4
 0x00,0x00,0x00,0x44,0xA4,0xA8,0xA8,0xA8,0x54,0x1A,0x2A,0x2A,0x2A,0x44,0x00,0x00,//"%",5
 0x00,0x00,0x00,0x30,0x48,0x48,0x48,0x50,0x6E,0xA4,0x94,0x88,0x89,0x76,0x00,0x00,//"&",6
 0x00,0x60,0x60,0x20,0xC0,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,//"'",7
 0x00,0x02,0x04,0x08,0x08,0x10,0x10,0x10,0x10,0x10,0x10,0x08,0x08,0x04,0x02,0x00,//"(",8
 0x00,0x40,0x20,0x10,0x10,0x08,0x08,0x08,0x08,0x08,0x08,0x10,0x10,0x20,0x40,0x00,//")",9
 0x00,0x00,0x00,0x00,0x10,0x10,0xD6,0x38,0x38,0xD6,0x10,0x10,0x00,0x00,0x00,0x00,//"*",10
 0x00,0x00,0x00,0x00,0x10,0x10,0x10,0x10,0xFE,0x10,0x10,0x10,0x10,0x00,0x00,0x00,//"+",11
 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x60,0x60,0x20,0xC0,//",",12
 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x7F,0x00,0x00,0x00,0x00,0x00,0x00,0x00,//"-",13
 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x60,0x60,0x00,0x00,//".",14
 0x00,0x00,0x01,0x02,0x02,0x04,0x04,0x08,0x08,0x10,0x10,0x20,0x20,0x40,0x40,0x00,//"/",15
 0x00,0x00,0x00,0x18,0x24,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x24,0x18,0x00,0x00,//"0",16
 0x00,0x00,0x00,0x10,0x70,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x7C,0x00,0x00,//"1",17
 0x00,0x00,0x00,0x3C,0x42,0x42,0x42,0x04,0x04,0x08,0x10,0x20,0x42,0x7E,0x00,0x00,//"2",18
 0x00,0x00,0x00,0x3C,0x42,0x42,0x04,0x18,0x04,0x02,0x02,0x42,0x44,0x38,0x00,0x00,//"3",19
 0x00,0x00,0x00,0x04,0x0C,0x14,0x24,0x24,0x44,0x44,0x7E,0x04,0x04,0x1E,0x00,0x00,//"4",20
 0x00,0x00,0x00,0x7E,0x40,0x40,0x40,0x58,0x64,0x02,0x02,0x42,0x44,0x38,0x00,0x00,//"5",21
 0x00,0x00,0x00,0x1C,0x24,0x40,0x40,0x58,0x64,0x42,0x42,0x42,0x24,0x18,0x00,0x00,//"6",22
 0x00,0x00,0x00,0x7E,0x44,0x44,0x08,0x08,0x10,0x10,0x10,0x10,0x10,0x10,0x00,0x00,//"7",23
 0x00,0x00,0x00,0x3C,0x42,0x42,0x42,0x24,0x18,0x24,0x42,0x42,0x42,0x3C,0x00,0x00,//"8",24
 0x00,0x00,0x00,0x18,0x24,0x42,0x42,0x42,0x26,0x1A,0x02,0x02,0x24,0x38,0x00,0x00,//"9",25
 0x00,0x00,0x00,0x00,0x00,0x00,0x18,0x18,0x00,0x00,0x00,0x00,0x18,0x18,0x00,0x00,//":",26
 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x10,0x00,0x00,0x00,0x00,0x00,0x10,0x10,0x20,//";",27
 0x00,0x00,0x00,0x02,0x04,0x08,0x10,0x20,0x40,0x20,0x10,0x08,0x04,0x02,0x00,0x00,//"<",28
 0x00,0x00,0x00,0x00,0x00,0x00,0xFE,0x00,0x00,0x00,0xFE,0x00,0x00,0x00,0x00,0x00,//"=",29
 0x00,0x00,0x00,0x40,0x20,0x10,0x08,0x04,0x02,0x04,0x08,0x10,0x20,0x40,0x00,0x00,//">",30
 0x00,0x00,0x00,0x3C,0x42,0x42,0x62,0x02,0x04,0x08,0x08,0x00,0x18,0x18,0x00,0x00,//"?",31
 0x00,0x00,0x00,0x38,0x44,0x5A,0xAA,0xAA,0xAA,0xAA,0xB4,0x42,0x44,0x38,0x00,0x00,//"@",32
 0x00,0x00,0x00,0x10,0x10,0x18,0x28,0x28,0x24,0x3C,0x44,0x42,0x42,0xE7,0x00,0x00,//"A",33
 0x00,0x00,0x00,0xF8,0x44,0x44,0x44,0x78,0x44,0x42,0x42,0x42,0x44,0xF8,0x00,0x00,//"B",34
 0x00,0x00,0x00,0x3E,0x42,0x42,0x80,0x80,0x80,0x80,0x80,0x42,0x44,0x38,0x00,0x00,//"C",35
 0x00,0x00,0x00,0xF8,0x44,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x44,0xF8,0x00,0x00,//"D",36
 0x00,0x00,0x00,0xFC,0x42,0x48,0x48,0x78,0x48,0x48,0x40,0x42,0x42,0xFC,0x00,0x00,//"E",37
 0x00,0x00,0x00,0xFC,0x42,0x48,0x48,0x78,0x48,0x48,0x40,0x40,0x40,0xE0,0x00,0x00,//"F",38
 0x00,0x00,0x00,0x3C,0x44,0x44,0x80,0x80,0x80,0x8E,0x84,0x44,0x44,0x38,0x00,0x00,//"G",39
 0x00,0x00,0x00,0xE7,0x42,0x42,0x42,0x42,0x7E,0x42,0x42,0x42,0x42,0xE7,0x00,0x00,//"H",40
 0x00,0x00,0x00,0x7C,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x7C,0x00,0x00,//"I",41
 0x00,0x00,0x00,0x3E,0x08,0x08,0x08,0x08,0x08,0x08,0x08,0x08,0x08,0x08,0x88,0xF0,//"J",42
 0x00,0x00,0x00,0xEE,0x44,0x48,0x50,0x70,0x50,0x48,0x48,0x44,0x44,0xEE,0x00,0x00,//"K",43
 0x00,0x00,0x00,0xE0,0x40,0x40,0x40,0x40,0x40,0x40,0x40,0x40,0x42,0xFE,0x00,0x00,//"L",44
 0x00,0x00,0x00,0xEE,0x6C,0x6C,0x6C,0x6C,0x54,0x54,0x54,0x54,0x54,0xD6,0x00,0x00,//"M",45
 0x00,0x00,0x00,0xC7,0x62,0x62,0x52,0x52,0x4A,0x4A,0x4A,0x46,0x46,0xE2,0x00,0x00,//"N",46
 0x00,0x00,0x00,0x38,0x44,0x82,0x82,0x82,0x82,0x82,0x82,0x82,0x44,0x38,0x00,0x00,//"O",47
 0x00,0x00,0x00,0xFC,0x42,0x42,0x42,0x42,0x7C,0x40,0x40,0x40,0x40,0xE0,0x00,0x00,//"P",48
 0x00,0x00,0x00,0x38,0x44,0x82,0x82,0x82,0x82,0x82,0xB2,0xCA,0x4C,0x38,0x06,0x00,//"Q",49
 0x00,0x00,0x00,0xFC,0x42,0x42,0x42,0x7C,0x48,0x48,0x44,0x44,0x42,0xE3,0x00,0x00,//"R",50
 0x00,0x00,0x00,0x3E,0x42,0x42,0x40,0x20,0x18,0x04,0x02,0x42,0x42,0x7C,0x00,0x00,//"S",51
 0x00,0x00,0x00,0xFE,0x92,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x38,0x00,0x00,//"T",52
 0x00,0x00,0x00,0xE7,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x3C,0x00,0x00,//"U",53
 0x00,0x00,0x00,0xE7,0x42,0x42,0x44,0x24,0x24,0x28,0x28,0x18,0x10,0x10,0x00,0x00,//"V",54
 0x00,0x00,0x00,0xD6,0x92,0x92,0x92,0x92,0xAA,0xAA,0x6C,0x44,0x44,0x44,0x00,0x00,//"W",55
 0x00,0x00,0x00,0xE7,0x42,0x24,0x24,0x18,0x18,0x18,0x24,0x24,0x42,0xE7,0x00,0x00,//"X",56
 0x00,0x00,0x00,0xEE,0x44,0x44,0x28,0x28,0x10,0x10,0x10,0x10,0x10,0x38,0x00,0x00,//"Y",57
 0x00,0x00,0x00,0x7E,0x84,0x04,0x08,0x08,0x10,0x20,0x20,0x42,0x42,0xFC,0x00,0x00,//"Z",58
 0x00,0x1E,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x1E,0x00,//"[",59
 0x00,0x00,0x40,0x40,0x20,0x20,0x10,0x10,0x10,0x08,0x08,0x04,0x04,0x04,0x02,0x02,//"\",60
 0x00,0x78,0x08,0x08,0x08,0x08,0x08,0x08,0x08,0x08,0x08,0x08,0x08,0x08,0x78,0x00,//"]",61
 0x00,0x1C,0x22,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,//"^",62
 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xFF,//"_",63
 0x00,0x60,0x60,0x20,0xC0,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,//"'",64
 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x3C,0x42,0x1E,0x22,0x42,0x42,0x3F,0x00,0x00,//"a",65
 0x00,0x00,0x00,0xC0,0x40,0x40,0x40,0x58,0x64,0x42,0x42,0x42,0x64,0x58,0x00,0x00,//"b",66
 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x1C,0x22,0x40,0x40,0x40,0x22,0x1C,0x00,0x00,//"c",67
 0x00,0x00,0x00,0x06,0x02,0x02,0x02,0x1E,0x22,0x42,0x42,0x42,0x26,0x1B,0x00,0x00,//"d",68
 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x3C,0x42,0x7E,0x40,0x40,0x42,0x3C,0x00,0x00,//"e",69
 0x00,0x00,0x00,0x0F,0x11,0x10,0x10,0x7E,0x10,0x10,0x10,0x10,0x10,0x7C,0x00,0x00,//"f",70
 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x3E,0x44,0x44,0x38,0x40,0x3C,0x42,0x42,0x3C,//"g",71
 0x00,0x00,0x00,0xC0,0x40,0x40,0x40,0x5C,0x62,0x42,0x42,0x42,0x42,0xE7,0x00,0x00,//"h",72
 0x00,0x00,0x00,0x30,0x30,0x00,0x00,0x70,0x10,0x10,0x10,0x10,0x10,0x7C,0x00,0x00,//"i",73
 0x00,0x00,0x00,0x0C,0x0C,0x00,0x00,0x1C,0x04,0x04,0x04,0x04,0x04,0x04,0x44,0x78,//"j",74
 0x00,0x00,0x00,0xC0,0x40,0x40,0x40,0x4E,0x48,0x50,0x68,0x48,0x44,0xEE,0x00,0x00,//"k",75
 0x00,0x00,0x00,0x70,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x7C,0x00,0x00,//"l",76
 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xFE,0x49,0x49,0x49,0x49,0x49,0xED,0x00,0x00,//"m",77
 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xDC,0x62,0x42,0x42,0x42,0x42,0xE7,0x00,0x00,//"n",78
 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x3C,0x42,0x42,0x42,0x42,0x42,0x3C,0x00,0x00,//"o",79
 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xD8,0x64,0x42,0x42,0x42,0x44,0x78,0x40,0xE0,//"p",80
 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x1E,0x22,0x42,0x42,0x42,0x22,0x1E,0x02,0x07,//"q",81
 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xEE,0x32,0x20,0x20,0x20,0x20,0xF8,0x00,0x00,//"r",82
 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x3E,0x42,0x40,0x3C,0x02,0x42,0x7C,0x00,0x00,//"s",83
 0x00,0x00,0x00,0x00,0x00,0x10,0x10,0x7C,0x10,0x10,0x10,0x10,0x10,0x0C,0x00,0x00,//"t",84
 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xC6,0x42,0x42,0x42,0x42,0x46,0x3B,0x00,0x00,//"u",85
 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xE7,0x42,0x24,0x24,0x28,0x10,0x10,0x00,0x00,//"v",86
 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xD7,0x92,0x92,0xAA,0xAA,0x44,0x44,0x00,0x00,//"w",87
 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x6E,0x24,0x18,0x18,0x18,0x24,0x76,0x00,0x00,//"x",88
 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xE7,0x42,0x24,0x24,0x28,0x18,0x10,0x10,0xE0,//"y",89
 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x7E,0x44,0x08,0x10,0x10,0x22,0x7E,0x00,0x00,//"z",90
 0x00,0x03,0x04,0x04,0x04,0x04,0x04,0x08,0x04,0x04,0x04,0x04,0x04,0x04,0x03,0x00,//"{",91
 0x08,0x08,0x08,0x08,0x08,0x08,0x08,0x08,0x08,0x08,0x08,0x08,0x08,0x08,0x08,0x08,//"|",92
 0x00,0x60,0x10,0x10,0x10,0x10,0x10,0x08,0x10,0x10,0x10,0x10,0x10,0x10,0x60,0x00,//"}",93
 0x30,0x4C,0x43,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,//"~",94
};

static sc_spi_lcd_write_t lcd_st7735s_spi_init_data_128_128[]=
{
    { SC_LCD_CMD, 0x11, 0 },
    { SC_LCD_DELAY, 120, 0 },
    { SC_LCD_CMD, 0xB1, 3 },
    { SC_LCD_DATA, 0x05, 0 },
    { SC_LCD_DATA, 0x3A, 0 },
    { SC_LCD_DATA, 0x3A, 0 },
    { SC_LCD_CMD, 0xB2, 3 },
    { SC_LCD_DATA, 0x05, 0 },
    { SC_LCD_DATA, 0x3A, 0 },
    { SC_LCD_DATA, 0x3A, 0 },
    { SC_LCD_CMD, 0xB3, 6 },
    { SC_LCD_DATA, 0x05, 0 },
    { SC_LCD_DATA, 0x3A, 0 },
    { SC_LCD_DATA, 0x3A, 0 },
    { SC_LCD_DATA, 0x05, 0 },
    { SC_LCD_DATA, 0x3A, 0 },
    { SC_LCD_DATA, 0x3A, 0 },
    { SC_LCD_CMD, 0xB4, 1 },
    { SC_LCD_DATA, 0x03, 0 },
    { SC_LCD_CMD, 0xC0, 3 },
    { SC_LCD_DATA, 0x62, 0 },
    { SC_LCD_DATA, 0x02, 0 },
    { SC_LCD_DATA, 0x04, 0 },
    { SC_LCD_CMD, 0xC1, 1 },
    { SC_LCD_DATA, 0xC0, 0 },
    { SC_LCD_CMD, 0xC2, 2 },
    { SC_LCD_DATA, 0x0C, 0 },
    { SC_LCD_DATA, 0x00, 0 },
    { SC_LCD_CMD, 0xC3, 2 },
    { SC_LCD_DATA, 0x8C, 0 },
    { SC_LCD_DATA, 0x6A, 0 },
    { SC_LCD_CMD, 0xC4, 2 },
    { SC_LCD_DATA, 0x8B, 0 },
    { SC_LCD_DATA, 0xEE, 0 },
    { SC_LCD_CMD, 0xC5, 1 },
    { SC_LCD_DATA, 0x12, 0 },
    { SC_LCD_CMD, 0xE0, 16 },
    { SC_LCD_DATA, 0x03, 0 },
    { SC_LCD_DATA, 0x1B, 0 },
    { SC_LCD_DATA, 0x12, 0 },
    { SC_LCD_DATA, 0x11, 0 },
    { SC_LCD_DATA, 0x3F, 0 },
    { SC_LCD_DATA, 0x3A, 0 },
    { SC_LCD_DATA, 0x32, 0 },
    { SC_LCD_DATA, 0x34, 0 },
    { SC_LCD_DATA, 0x2F, 0 },
    { SC_LCD_DATA, 0x2B, 0 },
    { SC_LCD_DATA, 0x30, 0 },
    { SC_LCD_DATA, 0x3A, 0 },
    { SC_LCD_DATA, 0x00, 0 },
    { SC_LCD_DATA, 0x01, 0 },
    { SC_LCD_DATA, 0x02, 0 },
    { SC_LCD_DATA, 0x05, 0 },
    { SC_LCD_CMD, 0xE1, 16 },
    { SC_LCD_DATA, 0x03, 0 },
    { SC_LCD_DATA, 0x1B, 0 },
    { SC_LCD_DATA, 0x12, 0 },
    { SC_LCD_DATA, 0x11, 0 },
    { SC_LCD_DATA, 0x32, 0 },
    { SC_LCD_DATA, 0x2F, 0 },
    { SC_LCD_DATA, 0x2A, 0 },
    { SC_LCD_DATA, 0x2F, 0 },
    { SC_LCD_DATA, 0x2E, 0 },
    { SC_LCD_DATA, 0x2C, 0 },
    { SC_LCD_DATA, 0x35, 0 },
    { SC_LCD_DATA, 0x3F, 0 },
    { SC_LCD_DATA, 0x00, 0 },
    { SC_LCD_DATA, 0x00, 0 },
    { SC_LCD_DATA, 0x01, 0 },
    { SC_LCD_DATA, 0x05, 0 },
    //{ SC_LCD_CMD, 0xFC, 1 },
    //{ SC_LCD_DATA, 0x8C, 0 },
    { SC_LCD_CMD, 0x3A, 1 },
    { SC_LCD_DATA, 0x05, 0 }, /* Format is RGB565, if for RGB666, 0x05 -> 0x06 */
    { SC_LCD_CMD, 0x36, 1 },
    { SC_LCD_DATA, 0x60, 0 }, //C0  //A0
    { SC_LCD_CMD, 0x29, 0 },
    { SC_LCD_DELAY, 120, 0 },
    { SC_LCD_CMD, 0x2C, 0 },
    { SC_LCD_DELAY, 120, 0 },
};

static sc_spi_lcd_write_t lcd_st7735s_spi_init_data_128_160[] =
{
    { SC_LCD_CMD, 0x11, 0 },
    { SC_LCD_DELAY, 120, 0 },
    { SC_LCD_CMD, 0xB1, 3 },
    { SC_LCD_DATA, 0x05, 0 },
    { SC_LCD_DATA, 0x3A, 0 },
    { SC_LCD_DATA, 0x3A, 0 },
    { SC_LCD_CMD, 0xB2, 3 },
    { SC_LCD_DATA, 0x05, 0 },
    { SC_LCD_DATA, 0x3A, 0 },
    { SC_LCD_DATA, 0x3A, 0 },
    { SC_LCD_CMD, 0xB3, 6 },
    { SC_LCD_DATA, 0x05, 0 },
    { SC_LCD_DATA, 0x3A, 0 },
    { SC_LCD_DATA, 0x3A, 0 },
    { SC_LCD_DATA, 0x05, 0 },
    { SC_LCD_DATA, 0x3A, 0 },
    { SC_LCD_DATA, 0x3A, 0 },
    { SC_LCD_CMD, 0xB4, 1 },
    { SC_LCD_DATA, 0x03, 0 },
    { SC_LCD_CMD, 0xC0, 3 },
    { SC_LCD_DATA, 0x62, 0 },
    { SC_LCD_DATA, 0x02, 0 },
    { SC_LCD_DATA, 0x04, 0 },
    { SC_LCD_CMD, 0xC1, 1 },
    { SC_LCD_DATA, 0xC0, 0 },
    { SC_LCD_CMD, 0xC2, 2 },
    { SC_LCD_DATA, 0x0D, 0 },
    { SC_LCD_DATA, 0x00, 0 },
    { SC_LCD_CMD, 0xC3, 2 },
    { SC_LCD_DATA, 0x8D, 0 },
    { SC_LCD_DATA, 0x6A, 0 },
    { SC_LCD_CMD, 0xC4, 2 },
    { SC_LCD_DATA, 0x8D, 0 },
    { SC_LCD_DATA, 0xEE, 0 },
    { SC_LCD_CMD, 0xC5, 1 },
    { SC_LCD_DATA, 0x12, 0 },
    { SC_LCD_CMD, 0xE0, 16 },
    { SC_LCD_DATA, 0x03, 0 },
    { SC_LCD_DATA, 0x1B, 0 },
    { SC_LCD_DATA, 0x12, 0 },
    { SC_LCD_DATA, 0x11, 0 },
    { SC_LCD_DATA, 0x3F, 0 },
    { SC_LCD_DATA, 0x3A, 0 },
    { SC_LCD_DATA, 0x32, 0 },
    { SC_LCD_DATA, 0x34, 0 },
    { SC_LCD_DATA, 0x2F, 0 },
    { SC_LCD_DATA, 0x2B, 0 },
    { SC_LCD_DATA, 0x30, 0 },
    { SC_LCD_DATA, 0x3A, 0 },
    { SC_LCD_DATA, 0x00, 0 },
    { SC_LCD_DATA, 0x01, 0 },
    { SC_LCD_DATA, 0x02, 0 },
    { SC_LCD_DATA, 0x05, 0 },
    { SC_LCD_CMD, 0xE1, 16 },
    { SC_LCD_DATA, 0x03, 0 },
    { SC_LCD_DATA, 0x1B, 0 },
    { SC_LCD_DATA, 0x12, 0 },
    { SC_LCD_DATA, 0x11, 0 },
    { SC_LCD_DATA, 0x32, 0 },
    { SC_LCD_DATA, 0x2F, 0 },
    { SC_LCD_DATA, 0x2A, 0 },
    { SC_LCD_DATA, 0x2F, 0 },
    { SC_LCD_DATA, 0x2E, 0 },
    { SC_LCD_DATA, 0x2C, 0 },
    { SC_LCD_DATA, 0x35, 0 },
    { SC_LCD_DATA, 0x3F, 0 },
    { SC_LCD_DATA, 0x00, 0 },
    { SC_LCD_DATA, 0x00, 0 },
    { SC_LCD_DATA, 0x01, 0 },
    { SC_LCD_DATA, 0x05, 0 },
    { SC_LCD_CMD, 0xFC, 1 },
    { SC_LCD_DATA, 0x8C, 0 },
    { SC_LCD_CMD, 0x3A, 1 },
    { SC_LCD_DATA, 0x05, 0 },   /* Format is RGB565, if for RGB666, 0x05 -> 0x06 */
    { SC_LCD_CMD, 0x36, 1 },
    { SC_LCD_DATA, 0xC0, 0 },   //C0  //A0
    { SC_LCD_CMD, 0x29, 0 },
    { SC_LCD_DELAY, 100, 0 },
};

static sc_spi_lcd_write_t lcd_st7789v_spi_init_data_240_240[] =
{
    {SC_LCD_CMD, 0x11, 0},
    {SC_LCD_DELAY, 120, 0},
    {SC_LCD_CMD, 0x36, 1},//
    {SC_LCD_DATA, 0x00, 0},
    {SC_LCD_CMD, 0x3A, 1},
    {SC_LCD_DATA, 0x55, 0},
    {SC_LCD_CMD, 0xB2, 5},
    {SC_LCD_DATA, 0x0C, 0},
    {SC_LCD_DATA, 0x0C, 0},
    {SC_LCD_DATA, 0x00, 0},
    {SC_LCD_DATA, 0x33, 0},
    {SC_LCD_DATA, 0x33, 0},
    //{SC_LCD_CMD, 0x3A, 1},
    //{SC_LCD_DATA, 0x06, 0}, //format is RGB666
    {SC_LCD_CMD, 0x3A, 1},
    {SC_LCD_DATA, 0x05, 0}, //format is RGB565
    {SC_LCD_CMD, 0xBB, 1},
    {SC_LCD_DATA, 0x1A, 0},
    {SC_LCD_CMD, 0xC0, 1},
    {SC_LCD_DATA, 0x2C, 0},
    {SC_LCD_CMD, 0xC2, 1},
    {SC_LCD_DATA, 0x01, 0},
    {SC_LCD_CMD, 0xC3, 1},
    {SC_LCD_DATA, 0x0F, 0},
    {SC_LCD_CMD, 0xC4, 1},
    {SC_LCD_DATA, 0x20, 0},
    {SC_LCD_CMD, 0xC6, 1},
    {SC_LCD_DATA, 0x0F, 0},
    {SC_LCD_CMD, 0xD0, 2},
    {SC_LCD_DATA, 0xA4, 0},
    {SC_LCD_DATA, 0xA1, 0},
    {SC_LCD_CMD, 0xE0, 14},
    {SC_LCD_DATA, 0xD0, 0},
    {SC_LCD_DATA, 0x13, 0},
    {SC_LCD_DATA, 0x1A, 0},
    {SC_LCD_DATA, 0x0A, 0},
    {SC_LCD_DATA, 0x0A, 0},
    {SC_LCD_DATA, 0x26, 0},
    {SC_LCD_DATA, 0x3F, 0},
    {SC_LCD_DATA, 0x54, 0},
    {SC_LCD_DATA, 0x54, 0},
    {SC_LCD_DATA, 0x18, 0},
    {SC_LCD_DATA, 0x14, 0},
    {SC_LCD_DATA, 0x14, 0},
    {SC_LCD_DATA, 0x30, 0},
    {SC_LCD_DATA, 0x33, 0},
    {SC_LCD_CMD, 0xE1, 14},
    {SC_LCD_DATA, 0xD0, 0},
    {SC_LCD_DATA, 0x13, 0},
    {SC_LCD_DATA, 0x1A, 0},
    {SC_LCD_DATA, 0x0A, 0},
    {SC_LCD_DATA, 0x0A, 0},
    {SC_LCD_DATA, 0x26, 0},
    {SC_LCD_DATA, 0x3F, 0},
    {SC_LCD_DATA, 0x54, 0},
    {SC_LCD_DATA, 0x54, 0},
    {SC_LCD_DATA, 0x1A, 0},
    {SC_LCD_DATA, 0x16, 0},
    {SC_LCD_DATA, 0x16, 0},
    {SC_LCD_DATA, 0x32, 0},
    {SC_LCD_DATA, 0x35, 0},
    {SC_LCD_CMD, 0x29, 0},
    {SC_LCD_DELAY, 120, 0},
};

static sc_spi_lcd_write_t lcd_st7789v_spi_init_data_240_320[] =
{
    {SC_LCD_CMD, 0x11, 0},
    {SC_LCD_DELAY, 120, 0},
    {SC_LCD_CMD, 0x36, 1},
    {SC_LCD_DATA, 0x00, 0},
    {SC_LCD_CMD, 0x3A, 1},
    {SC_LCD_DATA, 0x06, 0},
    {SC_LCD_CMD, 0x35, 1},
    {SC_LCD_DATA, 0x00, 0},
    {SC_LCD_CMD, 0xC7, 1},
    {SC_LCD_DATA, 0x00, 0},
    {SC_LCD_CMD, 0xCC, 1},
    {SC_LCD_DATA, 0x09, 0},
    {SC_LCD_CMD, 0xB2, 5},
    {SC_LCD_DATA, 0x0C, 0},
    {SC_LCD_DATA, 0x0C, 0},
    {SC_LCD_DATA, 0x00, 0},
    {SC_LCD_DATA, 0x33, 0},
    {SC_LCD_DATA, 0x33, 0},
    //{SC_LCD_CMD, 0x3A, 1},
    //{SC_LCD_DATA, 0x06, 0}, //format is RGB666
    {SC_LCD_CMD, 0x3A, 1},
    {SC_LCD_DATA, 0x05, 0}, //format is RGB565
    {SC_LCD_CMD, 0x36, 1},
    {SC_LCD_DATA, 0x00, 0},
    {SC_LCD_CMD, 0xB7, 1},
    {SC_LCD_DATA, 0x35, 0}, //vgh=13.26 vgl=-12.5
    {SC_LCD_CMD, 0xBB, 1},
    {SC_LCD_DATA, 0x36, 0},
    {SC_LCD_CMD, 0xC0, 1},
    {SC_LCD_DATA, 0x2C, 0},
    {SC_LCD_CMD, 0xC2, 1},
    {SC_LCD_DATA, 0x01, 0},
    {SC_LCD_CMD, 0xC3, 1},
    {SC_LCD_DATA, 0x0D, 0},
    {SC_LCD_CMD, 0xC4, 1},
    {SC_LCD_DATA, 0x20, 0},
    {SC_LCD_CMD, 0xC6, 1},
    {SC_LCD_DATA, 0x15, 0},
    {SC_LCD_CMD, 0xD0, 2},
    {SC_LCD_DATA, 0xA4, 0},
    {SC_LCD_DATA, 0xA1, 0},
    {SC_LCD_CMD, 0xE0, 14},
    {SC_LCD_DATA, 0xD0, 0},
    {SC_LCD_DATA, 0x17, 0},
    {SC_LCD_DATA, 0x19, 0},
    {SC_LCD_DATA, 0x04, 0},
    {SC_LCD_DATA, 0x03, 0},
    {SC_LCD_DATA, 0x04, 0},
    {SC_LCD_DATA, 0x32, 0},
    {SC_LCD_DATA, 0x41, 0},
    {SC_LCD_DATA, 0x43, 0},
    {SC_LCD_DATA, 0x09, 0},
    {SC_LCD_DATA, 0x14, 0},
    {SC_LCD_DATA, 0x12, 0},
    {SC_LCD_DATA, 0x33, 0},
    {SC_LCD_DATA, 0x2C, 0},
    {SC_LCD_CMD, 0xE1, 14},
    {SC_LCD_DATA, 0xD0, 0},
    {SC_LCD_DATA, 0x18, 0},
    {SC_LCD_DATA, 0x17, 0},
    {SC_LCD_DATA, 0x04, 0},
    {SC_LCD_DATA, 0x03, 0},
    {SC_LCD_DATA, 0x04, 0},
    {SC_LCD_DATA, 0x31, 0},
    {SC_LCD_DATA, 0x46, 0},
    {SC_LCD_DATA, 0x43, 0},
    {SC_LCD_DATA, 0x09, 0},
    {SC_LCD_DATA, 0x14, 0},
    {SC_LCD_DATA, 0x13, 0},
    {SC_LCD_DATA, 0x31, 0},
    {SC_LCD_DATA, 0x2D, 0},
    {SC_LCD_CMD, 0x29, 0 },
    {SC_LCD_DELAY, 100, 0 },
};

static sc_spi_lcd_write_t lcd_st7567a_spi_init_data[] =
{
    {SC_LCD_CMD, 0xAE, 0 },
    {SC_LCD_CMD, 0xA2, 0 },
    {SC_LCD_CMD, 0xA0, 0 },
    {SC_LCD_CMD, 0xC0, 0 },
    {SC_LCD_CMD, 0x24, 0 },
    {SC_LCD_CMD, 0x81, 0 },
    {SC_LCD_CMD, 0x30, 0 },
    {SC_LCD_CMD, 0x2F, 0 },
    {SC_LCD_CMD, 0xA6, 0 },
    {SC_LCD_CMD, 0xA4, 0 },
    {SC_LCD_CMD, 0x40, 0 },
    {SC_LCD_CMD, 0xB0, 0 },
    {SC_LCD_CMD, 0x10, 0 },
    {SC_LCD_CMD, 0x01, 0 },
    //{SC_LCD_CMD, 0xA4, 0 },
    //{SC_LCD_CMD, 0xAF, 0 },
};

static sc_spi_lcd_write_t lcd_st7796u_spi_init_data_320_480[] =
{
    {SC_LCD_CMD, 0x11, 0},
    {SC_LCD_DELAY, 120, 0},

    {SC_LCD_CMD, 0xf0, 1},
    {SC_LCD_DATA, 0xc3, 0},

    {SC_LCD_CMD, 0xf0, 1},
    {SC_LCD_DATA, 0x96, 0},

    {SC_LCD_CMD, 0x36, 1}, //Landscape display
    {SC_LCD_DATA, 0x38, 0},

    {SC_LCD_CMD, 0x3a, 1},
    {SC_LCD_DATA, 0x05, 0}, //565

    {SC_LCD_CMD, 0xb4, 1},
    {SC_LCD_DATA, 0x01, 0},

    {SC_LCD_CMD, 0xb7, 1},
    {SC_LCD_DATA, 0xc6, 0},

    {SC_LCD_CMD, 0xe8, 8},
    {SC_LCD_DATA, 0x40, 0},
    {SC_LCD_DATA, 0x8a, 0},
    {SC_LCD_DATA, 0x00, 0},
    {SC_LCD_DATA, 0x00, 0},
    {SC_LCD_DATA, 0x29, 0},
    {SC_LCD_DATA, 0x19, 0},
    {SC_LCD_DATA, 0xa5, 0},
    {SC_LCD_DATA, 0x33, 0},


    {SC_LCD_CMD, 0xc1, 1},
    {SC_LCD_DATA, 0x06, 0},

    {SC_LCD_CMD, 0xc2, 1},
    {SC_LCD_DATA, 0xa5, 0},

    {SC_LCD_CMD, 0xc5, 1},
    {SC_LCD_DATA, 0x25, 0},


    {SC_LCD_CMD, 0xe0, 14},
    {SC_LCD_DATA, 0xf0, 0},
    {SC_LCD_DATA, 0x00, 0},
    {SC_LCD_DATA, 0x03, 0},
    {SC_LCD_DATA, 0x0b, 0},
    {SC_LCD_DATA, 0x0c, 0},
    {SC_LCD_DATA, 0x29, 0},
    {SC_LCD_DATA, 0x2e, 0},
    {SC_LCD_DATA, 0x44, 0},
    {SC_LCD_DATA, 0x41, 0},
    {SC_LCD_DATA, 0x17, 0},
    {SC_LCD_DATA, 0x11, 0},
    {SC_LCD_DATA, 0x13, 0},
    {SC_LCD_DATA, 0x16, 0},
    {SC_LCD_DATA, 0x1b, 0},

    {SC_LCD_CMD, 0xe1, 14},
    {SC_LCD_DATA, 0xf0, 0},
    {SC_LCD_DATA, 0x00, 0},
    {SC_LCD_DATA, 0x02, 0},
    {SC_LCD_DATA, 0x06, 0},
    {SC_LCD_DATA, 0x06, 0},
    {SC_LCD_DATA, 0x24, 0},
    {SC_LCD_DATA, 0x2a, 0},
    {SC_LCD_DATA, 0x43, 0},
    {SC_LCD_DATA, 0x3e, 0},
    {SC_LCD_DATA, 0x2d, 0},
    {SC_LCD_DATA, 0x1a, 0},
    {SC_LCD_DATA, 0x16, 0},
    {SC_LCD_DATA, 0x13, 0},
    {SC_LCD_DATA, 0x17, 0},

    {SC_LCD_CMD, 0xf0, 1},
    {SC_LCD_DATA, 0x3c, 0},

    {SC_LCD_CMD, 0xf0, 1},
    {SC_LCD_DATA, 0x69, 0},

    {SC_LCD_DELAY, 120, 0 },

    {SC_LCD_CMD, 0x29, 0},
    { SC_LCD_DELAY, 120, 0 },
    {SC_LCD_CMD, 0x2c, 0},
    { SC_LCD_DELAY, 120, 0 },
};

static sc_lcd_data_t lcd_st7735s_spi_data_128_128 = {
    .init_para = lcd_st7735s_spi_init_data_128_128,
    .init_para_len = sizeof(lcd_st7735s_spi_init_data_128_128)/sizeof(sc_spi_lcd_write_t),
    .width = 128,
    .height = 128,
    .te_signal = SC_LCD_CAP_NOTE,
    .spi_clk = SC_SPI_LCD_CLK_26M,
    .lcd_id = 0x7c89f0,
    .format = SC_SPI_FORMAT_RGB565,
    .sample_edge = SC_SPI_EDGE_RISING,
    .endian = SC_SPI_ENDIAN_MSB,
    .data_line = 1,
    .line_num = 4,
};

static sc_lcd_data_t lcd_st7735s_spi_data_128_160 = {
    .init_para = lcd_st7735s_spi_init_data_128_160,
    .init_para_len = sizeof(lcd_st7735s_spi_init_data_128_160)/sizeof(sc_spi_lcd_write_t),
    .width = 128,
    .height = 160,
    .te_signal = SC_LCD_CAP_NOTE,
    .spi_clk = SC_SPI_LCD_CLK_26M,
    .lcd_id = 0x7c89f0,
    .format = SC_SPI_FORMAT_RGB565,
    .sample_edge = SC_SPI_EDGE_RISING,
    .endian = SC_SPI_ENDIAN_MSB,
    .data_line = 1,
    .line_num = 4,
};

static sc_lcd_data_t lcd_st7789v_spi_data_240_240 = {
    .init_para = lcd_st7789v_spi_init_data_240_240,
    .init_para_len = sizeof(lcd_st7789v_spi_init_data_240_240)/sizeof(sc_spi_lcd_write_t),
    .width = 240,
    .height = 240,
    .te_signal = SC_LCD_CAP_NOTE,
    .spi_clk = SC_SPI_LCD_CLK_26M,
    .lcd_id = 0x858552,
    .format = SC_SPI_FORMAT_RGB565,
    .sample_edge = SC_SPI_EDGE_RISING,
    .endian = SC_SPI_ENDIAN_MSB,
    .data_line = 1,
    .line_num = 4,
};

static sc_lcd_data_t lcd_st7789v_spi_data_240_320 = {
    .init_para = lcd_st7789v_spi_init_data_240_320,
    .init_para_len = sizeof(lcd_st7789v_spi_init_data_240_320)/sizeof(sc_spi_lcd_write_t),
    .width = 240,
    .height = 320,
    .te_signal = SC_LCD_CAP_NOTE,
    .spi_clk = SC_SPI_LCD_CLK_26M,
    .lcd_id = 0x858552,
    .format = SC_SPI_FORMAT_RGB565,
    .sample_edge = SC_SPI_EDGE_RISING,
    .endian = SC_SPI_ENDIAN_MSB,
    .data_line = 1,
    .line_num = 4,
};

static sc_lcd_data_t lcd_st7567a_spi_data = {
    .init_para = lcd_st7567a_spi_init_data,
    .init_para_len = sizeof(lcd_st7567a_spi_init_data)/sizeof(sc_spi_lcd_write_t),
    .width = 128,
    .height = 64,
    .te_signal = SC_LCD_CAP_NOTE,
    .spi_clk = SC_SPI_LCD_CLK_26M,
    .lcd_id = 0x0,
    .format = SC_SPI_FORMAT_RGB565,
    .sample_edge = SC_SPI_EDGE_RISING,
    .endian = SC_SPI_ENDIAN_MSB,
    .data_line = 1,
    .line_num = 4,
};

static sc_lcd_data_t lcd_st7796u_spi_data_320_480 = {
    .init_para = lcd_st7796u_spi_init_data_320_480,
    .init_para_len = sizeof(lcd_st7796u_spi_init_data_320_480)/sizeof(sc_spi_lcd_write_t),
    .width = 480,  //Landscape display, Opposite of length and width
    .height = 320,
    .te_signal = SC_LCD_CAP_NOTE,
    .spi_clk = SC_SPI_LCD_CLK_26M,
    .lcd_id = 0x0,
    .format = SC_SPI_FORMAT_RGB565,
    .sample_edge = SC_SPI_EDGE_RISING,
    .endian = SC_SPI_ENDIAN_MSB,
    .data_line = 1,
    .line_num = 4,
};

static sc_lcd_data_t *lcd_list[6] = {
    &lcd_st7735s_spi_data_128_128,
    &lcd_st7735s_spi_data_128_160,
    &lcd_st7789v_spi_data_240_240,
    &lcd_st7789v_spi_data_240_320,
    &lcd_st7567a_spi_data,
    &lcd_st7796u_spi_data_320_480,
};

typedef struct {
    char lcd_type;
    char lcd_bri;
    unsigned short lcd_width;
    unsigned short lcd_height;
}sc_lcd_info_t;

void get_lcd_info(sc_lcd_info_t *lcd);

//Color definition
#define   BLACK     0x0000
#define   NAVY      0x000F
#define   DGREEN    0x03E0
#define   DCYAN     0x03EF
#define   MAROON    0x7800
#define   PURPLE    0x780F
#define   OLIVE     0x7BE0
#define   LGRAY     0xC618
#define   DGRAY     0x7BEF
#define   BLUE      0x001F
#define   GREEN     0x07E0
#define   CYAN      0x07FF
#define   RED       0xF800
#define   MAGENTA   0xF81F
#define   YELLOW    0xFFE0
#define   WHITE     0xFFFF
#define GB_16_num 4
typedef struct typFNT_GB16
{
    unsigned char Index[2];
    unsigned char Msk[32];
}typFNT_GB16_t;

//Chinese characters, meaning "Welcome to use"
static const typFNT_GB16_t GB_16[] = {
{{"ª∂"},{0x00,0x80,0x00,0x80,0xFC,0x80,0x04,0xFC,0x05,0x04,0x49,0x08,0x2A,0x40,0x14,0x40,
0x10,0x40,0x28,0xA0,0x24,0xA0,0x45,0x10,0x81,0x10,0x02,0x08,0x04,0x04,0x08,0x02}},
{{"”≠"},{0x00,0x00,0x20,0x80,0x13,0x3C,0x12,0x24,0x02,0x24,0x02,0x24,0xF2,0x24,0x12,0x24,
0x12,0x24,0x12,0xB4,0x13,0x28,0x12,0x20,0x10,0x20,0x28,0x20,0x47,0xFE,0x00,0x00}},
{{" π"},{0x10,0x40,0x10,0x40,0x1F,0xFE,0x20,0x40,0x20,0x40,0x67,0xFC,0x64,0x44,0xA4,0x44,
0x27,0xFC,0x20,0x40,0x22,0x40,0x21,0x40,0x20,0x80,0x21,0x40,0x22,0x30,0x2C,0x0E}},
{{"”√"},{0x00,0x00,0x3F,0xF8,0x21,0x08,0x21,0x08,0x21,0x08,0x3F,0xF8,0x21,0x08,0x21,0x08,
0x21,0x08,0x3F,0xF8,0x21,0x08,0x21,0x08,0x21,0x08,0x41,0x08,0x41,0x28,0x80,0x10}},
};

//Chinese characters, meaning "card"
static const typFNT_GB16_t TN_GB_16[] = {
{{"ø®"},{0x00,0x40,0x40,0x40,0x40,0x40,0x40,0xFF,0xFE,0x48,0x48,0x48,0x4C,0x48,0x60,0x40,
0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xFF,0x7F,0x01,0x02,0x06,0x1C,0x08,0x00,0x00}},
{{"∆¨"},{0x00,0x00,0x00,0xFE,0xFC,0x10,0x10,0x10,0x10,0x1F,0x1E,0x90,0x10,0x10,0x18,0x10,
0x00,0x40,0x38,0x1F,0x07,0x01,0x01,0x01,0x01,0x01,0xFF,0xFF,0x01,0x00,0x00,0x00}},
};
#endif
