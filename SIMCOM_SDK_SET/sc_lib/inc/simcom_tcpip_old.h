...
enum socketCfgType{
	SOCKET_SYNC_CFG,  //TCP SYN packets retransmitted interval
	SOCKET_RXT_CFG,   //TCP data package retransmission interval
	SOCKET_MSS_CFG,   //Maximum Segment Size
	SOCKET_RECVWIN_CFG,//TCP Sliding Window.
	DNSTMR_CFG        //Set the DNS retry interval,The first interval cannot be less than 2 seconds
...
struct dataPackageRetryTmr{
	int dataPackageRetry;
	UINT8 backoffCfg[13];
};
struct dnsWaitTmr{
	int dnsMaxRetry;
...
#define sAPI_TcpipInetNtoa sAPI_TcpipInet_ntoa
...
#define sAPI_TcpipGetaddrinfoWithPcid sAPI_TcpipGetaddrinfo_with_pcid
...
#define sAPI_TcpipSocketWithCallback sAPI_TcpipSocket_with_callback


INT32 sAPI_TcpipGetErrno(void);
int sAPI_SocketConfigSet(enum socketCfgType cfg,void *opt);
int sAPI_SocketConfigGet(enum socketCfgType cfg,void *opt,int size);
#endif
